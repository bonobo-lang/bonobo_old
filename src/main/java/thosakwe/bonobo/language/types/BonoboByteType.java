...
public class BonoboByteType extends BonoboType {
    public static final BonoboByteType INSTANCE = new BonoboByteType();

    private BonoboByteType() {
    ...
    private BonoboType typeForArithmetic(String operator, BonoboType otherType, ParserRuleContext source) throws BonoboException {
        if (otherType.isAssignableTo(INSTANCE))
            return INSTANCE;
        else if (otherType.isAssignableTo(BonoboIntegerType.INSTANCE))
